...
const TEST_APK_ACTIVITY = "android.support.test.runner.AndroidJUnitRunner";
const INSTRUMENTATION_TARGET = `${TEST_APK_PKG}/${TEST_APK_ACTIVITY}`;
...
class EspressoRunner {
  ...
  async startSession (caps) {
    ...
    this.adb.shell(cmd).catch((e) => { // eslint-disable-line
      ...
      logger.info(`Could not start session: ${e}`);
      err = e;
    });
